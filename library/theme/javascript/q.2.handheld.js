...
// @benny @todo - these hardcoded figures seem risky in case of zoom or browser differences NOTE: They are overwritten if found on load##
var 
    htmlHeight = 0,
    bbHeight = 37,
    headerHeight = 80,
...
if( typeof jQuery !== 'undefined' ) {
    ...
    jQuery(document).ready(function() {	
        ...
        if(jQuery('body.single-grant').length ) {
            grant_single_reflow();
        }
        ...
        jQuery( ".navbar-toggle" ).click(function() {
            ...
            jQuery( '#q-club-nav' ).removeClass('menu_slideout').addClass('menu_slidein').show();
        });
        bind_menu_x_click('#mobile_close');
        jQuery( ".submen" ).click(function(e) {
            e.preventDefault();
...
function bind_menu_x_click( tgt ){
    jQuery( tgt ).click(function(e){
        jQuery( '#q-club-nav' ).removeClass('menu_slidein').addClass('menu_slideout');
        setTimeout(function(){
            jQuery('#q-club-nav').hide();
        }, 200);
...
function grant_single_reflow() {
    var $stuff_to_reflow = [
        ['ul.the-excerpt a.button', 'section.single-grant-nav', true ]
    
    ];
    jQuery($stuff_to_reflow).each(function() { reflow_generic( this[0],this[1], this[2] ); });
    
}
function reflow_generic( $SELECTOR, $TARGET, $PARENT ) {
    /*/ Careful, this grabs everything that fits the JQuery selector definitions, can move more than one element to more than one target
    /*/
    $SELECTOR = jQuery( $SELECTOR ); $TARGET = jQuery( $TARGET );
    if( $SELECTOR.length && $TARGET.length ) {
        if( typeof( $PARENT !== 'undefined' ) && $PARENT ) {
            jQuery($SELECTOR).remove();
            jQuery($TARGET).prepend($SELECTOR);
        } else {
            jQuery($SELECTOR).remove();
            jQuery($TARGET).after($SELECTOR);
        }
    }
