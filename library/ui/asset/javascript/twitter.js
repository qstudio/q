...
OG src: jquery.minitwitter.js - http://minitwitter.webdevdesigner.com/
OG src: Copyright © 2012 Olivier Bréchet

Load in Twitter feed with jQuery ##
    ...
    $.fn.q_twitter = function( options ) {
        ...
        function url() {
            ...
            if (o.favorite) {
                ...
                return p+'//api.twitter.com/1/statuses/home_timeline.json?screen_name='+o.username[0]+'&exclude_replies='+o.replies+'&count='+limit+'&contributor_details=true&include_entities=1&callback=?';
                ...
                //console.log(p+'//search.twitter.com/search.json?q='+encodeURIComponent(query)+'&rpp='+limit+'&lang='+o.language+'&page='+o.page+'&include_entities=1&callback=?');
                return p+'//search.twitter.com/search.json?q='+encodeURIComponent(query)+'&rpp='+limit+'&lang='+o.language+'&page='+o.page+'&include_entities=1&callback=?';
                //return p+'//api.twitter.com/1.1/search/tweets.json?q='+encodeURIComponent(query)+'&count='+limit+'&lang='+o.language+'&include_entities=1&callback=?';
        ...
        $.fn.extend({
            ...
            function fetchTweetsData ( object ) {
                ...
                if ( o.filter !== null ) {
                    ...
                    // pass tweet text to "str" variable ##
                    str = obj.finalText;
                    ...
                    if (
                        ...
                        /* passed */
                        console.log( "OK! // Date: "+obj.tweetTime+" // User: "+obj.screenName+" // Text: "+ obj.finalText );
                        
                    } else {
                        
                        /* failed */
                        console.log( "FAILED! // Date: "+obj.tweetTime+" // User: "+obj.screenName+" // Text: "+ obj.finalText );
                        return false;
                ...
                obj.open = '<li>';
                obj.avatar = '<div class="avatar"><a '+rel()+' '+target()+' class="mt_avatar" href="'+obj.userUrl+'"><img src="'+obj.image+'" alt="'+obj.realName+'\'s avatar" border="0" class="icon"/></a></div>';
                obj.header = '<div class="meta"><div class="mt_header"> <a '+rel()+' '+target()+' class="mt_user" href="'+obj.userUrl+'">'+obj.realName+'</a> <span class="mt_screen_name">@'+obj.screenName+'</span></div>';
                obj.textTweet = '<div class="mt_text">'+obj.finalText+'</div>';
                obj.footer = obj.retweet ? '<div class="mt_footer"><div class="time">'+obj.tweetTime+'</div><span class="image_r"></span>Retweeted by <a '+rel()+' '+target()+' class="mt_retweet" href="http://twitter.com/'+object.user.screen_name+'">'+object.user.name+'</a></div></div>' : '<div class="mt_footer"><div class="time">'+obj.tweetTime+'</div></div></div></div>';
                obj.close = '</li>';
            ...
            function display ( widget ) {
                fetch().done(function( res ) {
                    ...
                    for ( var i=0; i<tweets.length; i++ ) {
                        if ( tweets[i].textTweet ) {
                            $(widget).append(tweets[i].open+tweets[i].avatar+tweets[i].header+tweets[i].textTweet+tweets[i].footer+tweets[i].close);
                        }
                    }
                    ...
                    if ( o.complete ) {
                        if (eval("typeof " + o.complete + " == 'function'")) {
                            //console.log("run on-complete function: "+ o.complete);
                            eval(o.complete+'()');
                        }
                    }
