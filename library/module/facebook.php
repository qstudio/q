...
class facebook extends \Q {
    ...
    public static function run()
    {
		...
		\q\module::filter([
			...
			'name'		=> 'Q ~ Facebook Tag Manager ( See Analytics and Marketing section )',
			'selected'	=> true,
		]);
		...
		if ( 
			! isset( core\option::get('module')->facebook )
			|| true !== core\option::get('module')->facebook 
		){
    ...
    public static function filter_acf_fields( $array ) 
    {
		...
		array_push( $array['fields'], [
            'key' => 'field_q_option_facebook_appid',
            'label' => 'Facebook AppID',
            'name' => 'q_option_facebook_appid',
            'type' => 'text',
            'instructions' => 'Enter the AppID for share widgets',
            'required' => 0,
        ]);
