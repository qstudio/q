...
class google extends \Q {
    ...
    public static function run()
    {
		...
		\q\module::filter([
			...
			'name'		=> 'Q ~ Google Tag Manager ( See Analytics and Marketing section )',
			'selected'	=> true,
		]);
		...
		if ( 
			! isset( core\option::get('module')->google )
			|| true !== core\option::get('module')->google 
		){
...
<?php
    ...
    public static function map_enqueue_hook()
    {
        ...
        \wp_enqueue_script( 'acf-google-maps',  self::get_plugin_url( 'library/ui/asset/javascript/acf-google-maps.js' ), array( 'jquery' ), self::version, true );

        \wp_register_style( 'q-map-css', h::get( "ui/asset/css/q.map.css", 'return' ), '', self::version, 'all' );
